...
module.exports = input => {
  ...
  if (!['samtale', 'notat'].includes(data.documentType)) {
      ...
      contacts: [
        {
          ...
          Recno: (data.p360Contact && data.p360Contact.Recno) || undefined,
    ...
    if (data.sendCopyToGuardian && data.recipientCopies && data.recipientCopies.length > 0) {
      const recipients = data.recipientCopies.map(copy => formatRecipients(copy, data.p360Contact))
  ...
  if (Array.isArray(meta.contacts) && meta.contacts.length > 0) {
    meta.contacts = meta.contacts.map(contact => {
      if (contact.Recno === undefined) delete contact.Recno
      return contact
    })
  }
