...
module.exports = async data => {
  try {
    const lang = 'nb'
    logger('info', ['convert document', data._id, 'language', lang, 'start'])
    // TODO: add language support...
    const result = await convert(data, lang)
    // appends new document to the converted old document
    result.newDocument = data
    logger('info', ['convert document', data._id, 'language', lang, 'finish'])
    return result
  } catch (error) {
    logger('error', ['convert document', data._id, error])
    throw error
  }
}
