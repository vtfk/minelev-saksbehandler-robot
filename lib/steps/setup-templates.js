...
module.exports = data => {
  return new Promise(async (resolve, reject) => {
    ...
    data.documentTemplates.push(Object.assign(data.document, {archive: true, archiveMetadata: archiveMetadata({data: data, documentData: documentData})}))
    ...
    if (data.sendToDistribution === false && data.template !== 'samtale') {
      ...
      data.documentTemplates.push(Object.assign(document, {archive: true, archiveMetadata: {documentData: documentData}}))
    }
    ...
    if (data.sendToDistribution === true) {
      ...
      const documentData = await generateDocumentData(data.document)
      data.documentTemplates.push(Object.assign(data.document, {distribution: true, documentData: documentData}))
    }
