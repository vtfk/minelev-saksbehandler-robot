...
module.exports = async data => {
  ...
  if (data.dsf !== false) {
    ...
    archive.case = {
      ...
      title: 'Elevmappe',
      unofficialTitle: `Elevmappe - ${recipient.fullName || data.newDocument.student.name}`,
      type: 'elevmappe',
      accessCode: '13',
    ...
    archive.contacts = [
      {
        ...
        personalIdNumber: recipient.personalIdNumber || data.newDocument.student.personalIdNumber,
    ...
    if (data.sendCopyToGuardian !== false && data.guardiansFound > 0 && !['samtale', 'notat'].includes(data.documentTemplate)) {
      logger('info', ['setup-archive', 'parents add to contact', data.guardiansFound])
      const guardians = filterGuardian(data.dsf)
      const contacts = guardians.map(formatContactPerson)
      logger('info', ['setup-archive', 'parents add to contact', contacts.length, 'parents found'])
      if (contacts.length > 0) {
        archive.contacts = archive.contacts.concat(contacts)
      }
    } else {
      logger('info', ['setup-archive', 'parents add to contact', false])
    }
    ...
    logger('error', ['setup-archive', 'missing dsf data'])
  }
  return data
}
